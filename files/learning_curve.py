...
def main(): # called at end of file
	...
	# Tuning #############################################################################
	trainSetSize = len(trainSet["data"])
	currentTrainSetSize = [math.floor(trainSetSize*i/10) for i in range(1,11)]
	for i in range(10):
		accuracy = kNNAccuracy(trainSet["data"][:currentTrainSetSize[i]], testSet, k)
		print(str(currentTrainSetSize[i])+","+str(accuracy))
...
######################################################################################
def kNNAccuracy(trainingData, testSet_f, k):
